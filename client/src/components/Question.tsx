...
export default function Questions({ id, question, answer }: Props) {
  ...
  return (
    <div
      className="flex items-center w-[50%] justify-between cart relative max-lg:w-[calc(100vw-10vw)]"
      key={id}
    >
      <div className="flex items-center gap-5 w-full">
        <div className="button rounded bg-[#262626] font-bold">{id}</div>
        <div>
          <span className="font-bold">{question}</span>
          {isOpen ? (
            <div
              className={`answer overflow-hidden text-[#999999] transition-all duration-500 ease-in-out navbar ${
      ...
      <img
        ...
        src={Plus}
        alt="plus icon"
        className={`transition-transform cursor-pointer duration-300 w-[30px] h-[30px] object-contain ${
          isOpen ? "rotate-45" : ""
        }`}
      />
